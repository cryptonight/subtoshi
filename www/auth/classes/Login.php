...
class Login
{
    ...
    public function __construct()
    {
        ...
        if (isset($_POST["request_password_reset"]) && isset($_POST['user_email'])) {
            /*Begin added by Pierce Stegman*/
            // database query, getting all the info of the selected user
            if ($this->databaseConnection()) {
                $query_user = $this->db_connection->prepare('SELECT * FROM users WHERE user_email = :user_email');
                $query_user->bindValue(':user_email', trim($_POST['user_email']), PDO::PARAM_STR);
                $query_user->execute();
                // get result row (as an object)
                $result_row = $query_user->fetchObject();
                $returned_user_name = $result_row->user_name;
                if(empty($returned_user_name)){
                    $this->setPasswordResetDatabaseTokenAndSendMail($_POST['user_email']);
                }else{
                    $this->setPasswordResetDatabaseTokenAndSendMail($returned_user_name);
                }
            }else{
                $this->setPasswordResetDatabaseTokenAndSendMail($_POST['user_email']);
            }
            /*End added by Pierce Stegman*/
