...
require_once(realpath("/etc/subtoshi/passwords.php"));
//$sen_data is the variable in passwords.php
...
function generatePaymentId($coin){
    ...
    assert(in_array($coin, $cryptonote)); 
    
    $payment_ids = getPaymentIds($coin);
    
    assert(count($payment_ids) == 0);
    
    $bytes = openssl_random_pseudo_bytes(32, $cstrong);
    $new_payment_id = "" . bin2hex($bytes);
